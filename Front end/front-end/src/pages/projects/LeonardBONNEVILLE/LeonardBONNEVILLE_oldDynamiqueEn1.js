...
const code_recursif = `if recursive(n):
if n == 0:
    return 0
...
function affichageFigure(dataFig) {
    if (dataFig) {
        switch (dataFig.type) {
            case 0:
                return (
...
function affichageCode(dataCode, balise = []) {
    if (!dataCode) {
        return null;
    }
...
function affichageText(data, balise) {
...
function LeonardBONNEVILLE() {
    ...
    const [projetSite, setProjetSite] = useState(
        {
    ...
    const [projetGreenIT, setprojetGreenIT] = useState(
        {
    ...
    useEffect(() => {
        // L'appel API irait ici, setContent avec les données de l'API
        // setContent({});
    }, []);
